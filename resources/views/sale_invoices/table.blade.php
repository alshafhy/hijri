<table class="table table-striped table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('dashboard.sale-invoices.show', $invoice->id) }}" class="btn btn-sm btn-info">{{ __('View') }}</a>
                @can('sale_invoice.delete')
                <form action="{{ route('dashboard.sale-invoices.destroy', $invoice->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
        ...
        @empty
        <tr>
            <td colspan="11" class="text-center">{{ __('No invoices found') }}.</td>
        </tr>
        @endforelse
    </tbody>
</table>
